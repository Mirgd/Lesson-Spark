...
export function WorksheetBuilder({ plan }: { plan: LessonPlan }) {
  ...
  const run = async () => {
    if (slides.length === 0) {
      toast.error("أنشئ العرض أولاً — ورقة العمل مرتبطة بالشرائح.");
      return;
    }
    setBusy(true);
    try {
      ...
      // archive the generated questions into the bank for future lessons
      const added = addMany(
      ...
      toast.success(
        `تم توليد ورقة العمل — ${res.items.length} أنشطة${added ? ` · أُضيف ${added} سؤالاً للبنك` : ""}`,
      );
    } catch (e) {
      toast.error(reportAiError(e, "توليد ورقة العمل", "فشل توليد ورقة العمل"));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="card-elevated p-4">
      ...
      >
        ...
        {busy ? "جارٍ توليد ورقة العمل..." : "📝 ولّد ورقة عمل الطالب من الشرائح"}
      </button>
        ...
        <div className="mt-4 space-y-2">
            ...
            {items.map((it, i) => (
              <li key={`${it.slideIndex}-${i}`} className="rounded-lg border bg-card p-2">
                <div className="mb-1 flex items-center gap-2">
                  ...
                  >
                    {PHASE_LABELS[it.phase as keyof typeof PHASE_LABELS]?.ar ?? it.phase}
                  </span>
                ...
                <textarea
                  ...
                  onChange={(e) => patch(i, { questions: e.target.value.split("\n") })}
                  rows={2}
                  placeholder="سؤال في كل سطر"
                  className="w-full rounded border p-2 text-sm"
                />
                <textarea
                  value={(it.answers ?? []).join("\n")}
                  onChange={(e) => patch(i, { answers: e.target.value.split("\n") })}
                  rows={2}
                  placeholder="الإجابة النموذجية لكل سؤال (بنفس الترتيب)"
                  className="mt-1 w-full rounded border p-2 text-sm"
                  style={{ background: "#16794A0F" }}
                />
                <input
                  value={it.selfCheck}
                  onChange={(e) => patch(i, { selfCheck: e.target.value })}
                  placeholder="أستطيع أن..."
                  className="mt-1 w-full rounded border p-2 text-sm"
                />
              </li>
            ))}
          ...
          <div className="flex flex-wrap gap-2">
            <a
              ...
              style={{ background: TEAL }}
            >
              <Printer className="h-4 w-4" /> اطبع ورقة الطالب
            </a>
            <a
              href="/absent"
              target="_blank"
            ...
            >
              <UserX className="h-4 w-4" /> نسخة الطالب الغائب (شاملة)
            </a>
          </div>
        </div>
      )}
