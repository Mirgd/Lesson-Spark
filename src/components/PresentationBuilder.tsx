...
import { Loader2, Play, Trash2, Plus, ChevronRight, ChevronLeft, Pencil } from "lucide-react";
...
import { planLang, type LessonPlan, useCurriculum } from "@/lib/lesson-types";
...
export function PresentationBuilder({ plan }: { plan: LessonPlan }) {
  ...
  const downloadPptx = async () => {
    if (!slides.length) {
      toast.error(isArabic ? "أنشئ العرض أولاً" : "Generate the presentation first");

      return;
    }

    try {
      await downloadPresentationPptx(slides, plan.topic || "lesson-presentation");

      toast.success(isArabic ? "تم تحميل العرض التقديمي" : "Presentation downloaded successfully");
    } catch (error) {
      console.error("PowerPoint download failed:", error);

      toast.error(isArabic ? "تعذّر تحميل العرض التقديمي" : "Unable to download presentation");
    }
  };
  ...
  const run = async (file: File) => {
    ...
    setStatus(
      isArabic ? "جارٍ تحويل صفحات الكتاب إلى صور..." : "Converting book pages to images...",
    );
    ...
    try {
      ...
      if (!pages.length) {
        throw new Error(
          isArabic ? "لم يتم العثور على صفحات في ملف PDF." : "No pages were found in the PDF file.",
        );
      }
      ...
      setStatus(isArabic ? "جارٍ بناء محتوى العرض..." : "Building presentation content...");
      ...
      toast.error(
        reportAiError(
          error,
          isArabic ? "بناء العرض التقديمي" : "Presentation Builder",
          isArabic ? "فشل بناء العرض" : "Failed to build the presentation",
        ),
      );
    } finally {
      setBusy(false);
  ...
  const removeSlide = (index: number) => {
    setSlides(reindex(slides.filter((_, idx) => idx !== index)));
    ...
    setSlides(
      reindex([
        ...slides,
        {
          ...
          type: "blank",
          phase: "explain",
          title: isArabic ? "شريحة جديدة" : "New Slide",
          points: [""],
          question: "",
        } as Slide,
      ]),
   ...
   */
  const patch = (index: number, patchData: Partial<Slide>) => {
    setSlides(
   ...
   */
  const counts = slides.reduce<Record<string, number>>((acc, slide) => {
    acc[slide.phase] = (acc[slide.phase] ?? 0) + 1;
  ...
  return (
      ...
      {busy && (
        <div className="mt-4 rounded-lg border p-3">
          ...
          <div className="mt-2 flex justify-between text-xs text-muted-foreground">
            ...
            <span>{isArabic ? `${done} من ${total}` : `${done} of ${total}`}</span>
          </div>
        </div>
      )}
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            {Object.entries(counts).map(([phase, count]) => (
              <span
                ...
                style={{
                  background: PHASE_LABELS[phase as Slide["phase"]]?.color ?? "#888",
                }}
              >
                {phaseName(phase as Slide["phase"])} ×{count}
              </span>
            ))}
          </div>
              ...
              return (
                <li
                  ...
                  className={`rounded-lg border bg-card p-3 transition-colors ${
                    isEditingOpen ? "border-primary/40 ring-1 ring-primary/10" : ""
                  }`}
                >
                  {/* =========================
            رأس الشريحة
        ========================= */}
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span
                      ...
                      style={{
                        background: PHASE_LABELS[slide.phase]?.color ?? "#888",
                      }}
                    >
                    ...
                    <div className="min-w-0 flex-1 truncate px-1 text-sm font-bold text-primary">
                      {slide.title || (isArabic ? "بدون عنوان" : "Untitled Slide")}
                    </div>
                    ...
                    <button
                      ...
                      onClick={(event) => {
                        ...
                        setPreview(isPreviewOpen ? null : index);
                      ...
                      className={`rounded px-2 py-1 text-xs font-medium transition-colors ${
                        isPreviewOpen ? "bg-accent text-primary" : "hover:bg-accent"
                      }`}
                    >
                      {isArabic
                        ? isPreviewOpen
    ...
    ========================= */}
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-[11px] text-muted-foreground">
                          {isArabic ? `الشريحة ${index + 1}` : `Slide ${index + 1}`}
                        </span>
                      </div>
    ...
    ========================= */}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder={isArabic ? "اكتب عنوان الشريحة" : "Enter slide title"}
                        />
                      </div>
    ...
    ========================= */}
                      ...
                      {slide.type !== "cover" && (
                        <>
                          <div>
                            <label className="mb-1 block text-xs font-bold text-muted-foreground">
                              {isArabic ? "نقاط الشريحة" : "Slide Points"}
                            </label>
                            ...
                            <textarea
                              ...
                              rows={5}
                              placeholder={
                                isArabic ? "اكتب كل نقطة في سطر منفصل" : "Write one point per line"
                              }
    ...
    ========================= */}
                      ...
                      {slide.type === "homework" && (
                        <div>
                          <label className="mb-1 block text-xs font-bold text-muted-foreground">
                            {isArabic ? "محتوى الواجب المنزلي" : "Homework Content"}
                          </label>
                          ...
                          <textarea
                            ...
                            rows={4}
                            placeholder={
                              isArabic ? "اكتب الواجب المنزلي" : "Enter homework content"
                            }
                            className="w-full resize-y rounded-lg border bg-background px-3 py-2 text-sm outline-none focus:border-primary"
                          />
                        </div>
    ...
    ========================= */}
                      ...
                      <div className="rounded-xl border bg-background/70 p-3">
                        <label className="mb-2 block text-xs font-bold text-muted-foreground">
                          {isArabic ? "صورة الشريحة" : "Slide Image"}
                        </label>
                        ...
                        <input
                          ...
                          onChange={(event) => {
                            ...
                            reader.onload = () => {
                              ...
                              toast.success(
                                isArabic ? "تمت إضافة الصورة إلى الشريحة" : "Image added to slide",
                              );
                            };

                            reader.onerror = () => {
                              toast.error(isArabic ? "تعذّر قراءة الصورة" : "Unable to read image");
                            };
                          ...
                          <div className="mt-4 rounded-xl border bg-card p-3">
                            ...
                            <div className="flex justify-center rounded-lg bg-muted/30 p-2">
                              <img
                                ...
                                alt={slide.title || (isArabic ? "صورة الشريحة" : "Slide image")}
                              ...
                              <label className="cursor-pointer rounded-lg border px-3 py-1.5 text-xs font-bold hover:bg-accent">
                                ...
                                <input
                                  ...
                                  onChange={(event) => {
                                    ...
                                    reader.onerror = () => {
                                      toast.error(
                                        isArabic ? "تعذّر قراءة الصورة" : "Unable to read image",
                                      );
                                    };
...
========================= */}
                      ...
                      <div className="rounded-xl border bg-background/70 p-3">
                        <label className="mb-2 block text-xs font-bold text-muted-foreground">
                          {isArabic ? "🎬 فيديو الشريحة" : "🎬 Slide Video"}
                        </label>

                        <p className="mb-3 text-[11px] text-muted-foreground">
                          {isArabic
                            ? "يمكنك إضافة فيديو MP4 أو WebM ليظهر داخل هذه الشريحة."
                            : "You can add an MP4 or WebM video to this slide."}
                        </p>
                          ...
                          <input
                            ...
                            onChange={(event) => {
                              ...
                              if (file.size > maxSize) {
                                toast.error(
                                  isArabic
                                    ? "حجم الفيديو كبير جداً. اختر فيديو أقل من 50 MB."
                                    : "The video is too large. Choose a video smaller than 50 MB.",
                                );
                                return;
                              }
                              ...
                              reader.onload = () => {
                                ...
                                toast.success(
                                  isArabic
                                    ...
                                    : "Video added to slide",
                                );
                              };

                              reader.onerror = () => {
                                toast.error(
                                  isArabic ? "تعذّر قراءة الفيديو" : "Unable to read video",
                                );
                              };
                        ...
                        {slide.videoDataUrl && (
                          <div className="mt-3 rounded-xl border bg-card p-3">
                            <div className="mb-2 text-xs font-bold text-primary">
                              {slide.videoName || (isArabic ? "الفيديو المضاف" : "Added Video")}
                            </div>
                            ...
                            <div className="mt-3 flex justify-end">
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  toast.success(
                                    isArabic
                                      ? "تم حذف الفيديو من الشريحة"
                                      : "Video removed from slide",
                                  );
                                }}
                                className="rounded-lg border border-destructive/30 bg-destructive/5 px-3 py-1.5 text-xs font-bold text-destructive hover:bg-destructive/10"
                              >
...
========================= */}
                      ...
                      <div className="rounded-xl border bg-background/70 p-3">
                        <label className="mb-2 block text-xs font-bold text-muted-foreground">
                          {isArabic ? "🎵 صوت الشريحة" : "🎵 Slide Audio"}
                        </label>

                        <p className="mb-3 text-[11px] text-muted-foreground">
                          {isArabic
                            ? "يمكنك إضافة ملف MP3 أو WAV إلى هذه الشريحة."
                            : "You can add an MP3 or WAV audio file to this slide."}
                        </p>
                          ...
                          <input
                            ...
                            onChange={(event) => {
                              ...
                              if (file.size > maxSize) {
                                toast.error(
                                  isArabic
                                    ? "حجم الملف الصوتي كبير جداً. اختر ملفاً أقل من 20 MB."
                                    : "The audio file is too large. Choose a file smaller than 20 MB.",
                                );
                                return;
                              }
                              ...
                              if (!file.type.startsWith("audio/")) {
                                toast.error(
                                  isArabic
                                    ? "الملف المختار ليس ملفاً صوتياً."
                              ...
                              reader.onload = () => {
                                ...
                                toast.success(
                                  isArabic ? "تمت إضافة الصوت إلى الشريحة" : "Audio added to slide",
                                );
                              };
                        ...
                        {slide.audioDataUrl && (
                          <div className="mt-3 rounded-xl border bg-card p-3">
                            <div className="mb-2 text-xs font-bold text-primary">
                              {slide.audioName || (isArabic ? "الملف الصوتي" : "Audio File")}
                            </div>

                            <audio src={slide.audioDataUrl} controls className="w-full" />
                            ...
                            <div className="mt-3 flex justify-end">
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  toast.success(
                                    isArabic
                                      ? "تم حذف الصوت من الشريحة"
                                      : "Audio removed from slide",
                                  );
                                }}
                                className="rounded-lg border border-destructive/30 bg-destructive/5 px-3 py-1.5 text-xs font-bold text-destructive hover:bg-destructive/10"
                              >
                                {isArabic ? "حذف الصوت" : "Remove Audio"}
                              </button>
                            </div>
                          </div>
                        )}
    ...
    ========================= */}
                      ...
                      <div className="flex flex-wrap justify-end gap-2 border-t pt-3">
                        ...
                        >
                          {isArabic ? "معاينة التعديل" : "Preview Changes"}
                        </button>
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            toast.success(
                              isArabic ? "تم حفظ تعديلات الشريحة" : "Slide changes saved",
                            );
                          }}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              ...
              {isArabic ? "شريحة فارغة" : "Blank Slide"}
            </button>
            ...
            >
              {isArabic ? "تحميل PowerPoint" : "Download PowerPoint"}
            </button>
            ...
            >
              ...
              {isArabic ? "ابدأ العرض" : "Start Presentation"}
            </a>
          </div>
        </div>
      )}
