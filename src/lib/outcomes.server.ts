...
interface OutcomeInput { curriculum: string; topic: string; subject: string; grade: string; lang?: "ar" | "en" }
export interface OutcomeResult { outcomes: string[]; source: string }
  ...
  for (const candidate of candidates) {
    try {
      ...
      const outcomes = Array.isArray(parsed.outcomes)
        ? parsed.outcomes.filter((item): item is string => typeof item === "string" && item.trim().length > 0).map((item) => item.trim())
        : [];
      if (outcomes.length) return { outcomes, source: typeof parsed.source === "string" ? parsed.source.trim() : "" };
    } catch { /* use tolerant fallback */ }
  }
  const array = clean.match(/"outcomes"\s*:\s*\[([\s\S]*?)(\]|$)/);
  if (array) {
    const outcomes = [...array[1].matchAll(/"([^"]{4,})"/g)].map((match) => match[1].trim());
    if (outcomes.length) return { outcomes, source: clean.match(/"source"\s*:\s*"([^"]*)"/)?.[1]?.trim() ?? "" };
  }
  const source = clean.match(/"source"\s*:\s*"([^"]*)"/)?.[1]?.trim() ?? "";
  const strip = (line: string) =>
    line.replace(/^[\s\-*•◦●\d.)+:]+/, "").replace(/^["'“]|["'”],?$/g, "").trim();
  const noise = (line: string) =>
    !line || line.length < 5 || /^[{}[\],]+$/.test(line) || /^"?(outcomes|source)"?\s*:/i.test(line);

  // 1) أسطر النص
  const lines = clean.split(/\r?\n/).map(strip).filter((l) => !noise(l)).slice(0, 6);
  if (lines.length) return { outcomes: lines, source };

  // 2) جُمل داخل نص متصل
  const sentences = clean.split(/(?<=[.؟!،؛])\s+/).map(strip).filter((l) => !noise(l)).slice(0, 6);
...
${langInstruction(data.lang)}`;
  const raw = await callAiGateway({ messages: [{ role: "user", content: prompt }], maxTokens: 2500 });
