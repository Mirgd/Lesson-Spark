...
/** صفّ خطة في public.lesson_plans (مملوكة للمستخدم المسجَّل) */
...
/** صفّ خطة قديمة في public.open_plans (يُستخدم في لوحتي الإشراف والإدارة مؤقتاً) */
...
/** حزمة كاملة تُحفظ في قاعدة البيانات لكل خطة */
...
/** معرّف المستخدم المسجَّل حالياً — يُستخدم كمالك للخطة */
async function requireUserId(): Promise<string> {
  const { data, error } = await supabase.auth.getUser();
  if (error || !data.user) throw new Error("يجب تسجيل الدخول لحفظ الخطة");
  return data.user.id;
}
...
export function rowToBundle(row: PlanRow | OpenPlanRow): PlanBundle {
  ...
  return {
    plan: {
      ...
      contentLanguage: normalizeLang((row as { content_language?: unknown }).content_language),
...
/** يفتح خطة من قاعدة البيانات داخل مساحة العمل المحلية */
...
/** حفظ/تحديث خطة المستخدم المسجَّل (المفتاح: user_id + local_id) */
export async function upsertPlan(bundle: PlanBundle) {
  const userId = await requireUserId();
  const { error } = await supabase
    .from("lesson_plans")
    .upsert(planToRow(bundle, userId), { onConflict: "user_id,local_id" });
  if (error) throw error;
}

/** خطط المستخدم المسجَّل فقط */
...
/** جلب خطة واحدة بمعرّفها مع تقييد الملكية */
...
export async function duplicatePlan(id: string) {
  ...
  if (!row) throw new Error("لم يتم العثور على الخطة");
...
/** الخطط القديمة المفتوحة — تُقرأ فقط في لوحتي الإشراف والإدارة حتى ترحيلها */
export async function listOpenPlans(teacherName?: string) {
  let q = supabase.from("open_plans").select("*").order("updated_at", { ascending: false });
  if (teacherName) q = q.eq("teacher_name", teacherName);
